...
function Board(props) {
    ...
    slots.forEach((element, k) => {
        ...
        renderSlots.push( <
            div key = { k }
            ref = { slotsStates[k] === "running" ? scrollToRef : null } >
            <
            Slot label = { element.label }
            duration = { element.duration }
            key = { k }
    ...
    return ( <
            ...
            !modeEditor && ( <
                ...
                div > {
                    playingStates.indexOf(currentrunningState) != -1 && ( <
                        span >
                        <
                        button className = "square backStart"
                        desc = "Restart all"
                        onClick = {
                            () => { pause();
                                resetAllTimer();
                                setCurrentrunningState('stop') } } >
                        <
                        img src = { backStartButton }
                        alt = "Restart all"
                        title = "Restart all" / > { " " } <
                        /button> <
                        button className = "square back"
                        desc = "Restart current timer"
                        onClick = {
                            () => resetSlotTimer(currentRunningSlotIndex) } >
                        <
                        img src = { backButton }
                        alt = "Restart current timer"
...

                {
                    playingStates.indexOf(currentrunningState) != -1 && ( <
                        span >
                        <
                        button className = "square skip"
                        desc = "Skip current timer"
                        onClick = {
                            () => next() } >
                        <
                        img src = { forwardButton }
                        alt = "Skip current timer"
        ...
        <
        ShareConfigBox board = { boardControler }
        url = { shareURL }
        />

        <
        ConfigLoader board = { boardControler }
        prefixName = { TK_nameprefix }
        /> <
        /div>
    );
}
